/*
 * File:    frontend/src/app/(dashboard)/dashboard/admin/quizzes/new/page.tsx
 * Purpose: Super-admin quiz creator. Same 4-step wizard a teacher has (manual +
 *          AI from topic + AI from PDF) PLUS a school selector so the super admin
 *          can assign the quiz to any school's class. Unlike the teacher flow
 *          there is no approval step — it publishes immediately (authoring →
 *          publish). Posts to /quizzes/authoring/ with a target `school`.
 * Owner:   Pranav
 */
...
interface DraftQuestion {
  ...
  accepted_answers?: string[];     // short-answer only — model answer(s) / keywords
  points?: number;
}
...
// ─── Page ───────────────────────────────────────────────────────────────
export default function AdminQuizCreationWizard() {
  ...
  // Schools for the assignment selector (MAIN_ADMIN can assign to any school).
  useEffect(() => {
    (async () => {
  ...
  function basicValid(): string | null {
    if (!schoolId) return "Select the school to assign this quiz to.";
  ...
  // Create the quiz via the authoring adapter. `targetStatus` is what the quiz
  // should reach: DRAFT (save) or REVIEW (so the super admin can then publish).
  async function persistQuiz(targetStatus: "DRAFT" | "REVIEW"): Promise<string | null> {
    const token = await getToken();
    ...
    try {
      const quizRes = await fetch(`${API_BASE}/quizzes/authoring/`, {
        ...
        body: JSON.stringify({
          school: schoolId,                       // MAIN_ADMIN must name the target school
          ...
          questions: questions.map((q) => ({
            ...
            // Short-answer questions send [] options + accepted_answers; the
            // backend infers SHORT_ANSWER from the empty option list.
      ...
      if (!quizRes.ok) {
        ...
        toast(body?.detail ?? body?.school ?? `Failed to create quiz (${quizRes.status})`, "error");
        return null;
      }
  ...
  async function publishQuiz(id: string): Promise<boolean> {
    const token = await getToken();
    if (!token) return false;
    try {
      const res = await fetch(`${API_BASE}/quizzes/${id}/publish/`, {
        method: "POST",
  ...
  async function saveDraft() {
    const err = basicValid();
    if (err) { toast(err, "error"); return; }
    if (questions.length === 0) { toast("Add at least one question before saving.", "error"); return; }
    setSavingDraft(true);
  ...
  // Super admin: create then publish immediately (no approval queue).
  async function createAndPublish() {
    // Re-validate Basic Info here too: generating questions via AI jumps the
    // wizard to Step 2, which would otherwise let a user reach this button
    // without ever passing Step 1 (empty title/school → backend 400).
    const err = basicValid();
    if (err) { toast(err, "error"); setStep(1); return; }
    if (questions.length === 0) { toast("Add at least one question.", "error"); setStep(2); return; }
    setSubmitting(true);
    ...
    setSubmitting(false);
    if (published) {
      toast("Quiz created and published", "success");
    } else {
      toast("Quiz created but auto-publish failed — publish it from the Quiz Approval Panel.", "error");
    }
    router.push("/dashboard/admin/quizzes");
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="mt-1 text-sm text-[var(--muted-foreground)]">Build a quiz manually or with AI, assign it to any school, and publish instantly</p>
      </div>
          ...
          <AnimatePresence mode="wait">
            {step === 1 && (
              <motion.div key="s1" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
                <Step1BasicInfo basic={basic} onChange={setBasic} schools={schools} schoolId={schoolId} onSchool={setSchoolId} />
              </motion.div>
            )}
            {step === 2 && (
            ...
            {step === 4 && (
              <motion.div key="s4" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
                <Step4Review basic={basic} questions={questions} settings={settings} schoolName={schools?.find((s) => s.id === schoolId)?.name ?? "—"} onJump={setStep} />
              </motion.div>
            )}
          </AnimatePresence>
      ...
      {/* Cancel link */}
      <div>
        <Link href="/dashboard/admin/quizzes" className="text-xs font-semibold text-[var(--muted-foreground)] hover:text-primary">
          ← Discard and go back
        </Link>
      </div>
    </div>
...
function Step1BasicInfo({ basic, onChange, schools, schoolId, onSchool }: {
  basic: BasicInfo;
  ...
  return (
    <div className="p-6 md:p-7">
      <h2 className="text-lg font-bold tracking-tight text-[var(--foreground)]">School &amp; Basic Information</h2>
      <div className="mt-5 space-y-5">
        <Field label="Assign to School *">
          {schools === null ? (
            <div className="h-10 animate-pulse rounded-xl bg-[var(--muted)]" />
          ) : (
            <select value={schoolId} onChange={(e) => onSchool(e.target.value)} className={inputCls}>
  ...
  function addManual() {
    ...
    if (qType === "SHORT") {
      onAdd({
        ...
        // Optional model answer(s) — seed a provisional auto-score; the
        // school's teacher reviews short answers in the Feedback queue.
      ...
      <div className="rounded-2xl border border-[var(--border)] bg-[var(--muted)]/30 p-4 space-y-3">
        ...
        {qType === "MCQ" ? (
          ...
          <div className="space-y-1.5">
            ...
            <p className="text-[11px] text-[var(--muted-foreground)]">Students type a written answer. The school&apos;s teacher grades it in the Feedback queue; accepted answers (if any) just give a provisional auto-score.</p>
          </div>
        )}
...
function Step4Review({
  basic, questions, settings, schoolName, onJump,
}: {
  basic: BasicInfo;
  questions: DraftQuestion[];
  settings: Settings;
  ...
  return (
    <div className="space-y-5 p-6 md:p-7">
      <h2 className="text-lg font-bold tracking-tight text-[var(--foreground)]">Review &amp; Publish</h2>

      <ReviewCard title="School & Basic Info" onEdit={() => onJump(1)}>
        <ReviewItem k="School" v={schoolName} />
      ...
      <ReviewCard title={`Questions (${questions.length} · ${questions.reduce((s, q) => s + (q.points && q.points > 0 ? q.points : 1), 0)} marks)`} onEdit={() => onJump(2)}>
        {questions.length === 0 ? (
          <p className="text-sm text-red-600">No questions yet — add at least one before publishing.</p>
        ) : (
      ...
      <div className="rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-xs text-emerald-800">
        <strong>Create &amp; Publish</strong> publishes this quiz to the selected school immediately — no approval step. Students in the matching class will see it under My Quizzes.
      </div>
    </div>
  );
}
...
function AIGeneratorPanel({
  schoolId, defaultSubject, defaultDifficulty, defaultGrade, onGenerated,
}: {
  schoolId: string;
  defaultSubject: string;
  defaultDifficulty: Difficulty;
  ...
  const generate = useCallback(async () => {
    ...
    if (!schoolId) { toast("Select a school first (Step 1).", "error"); return; }
    setLoading(true);
    const token = await getToken();
    ...
    try {
      ...
      if (!res.ok) {
        ...
        toast(body?.detail ?? body?.school ?? `Generation failed (${res.status})`, "error");
        return;
      }
      const data = await res.json();
  ...
  }, [canGenerate, schoolId, mode, subject, defaultSubject, topic, grade, difficulty, count, pdfFile, onGenerated, toast]);

  return (
    <motion.div
      initial={{ opacity: 0, x: 12 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.4, delay: 0.1 }}
      className="h-fit overflow-hidden rounded-2xl border-2 border-primary/30 bg-gradient-to-br from-primary/5 via-white to-accent/5 shadow-sm dark:from-primary/10 dark:via-[var(--background)] dark:to-accent/10"
    >
      ...
      <div className="space-y-4 p-5">
        ...
        {mode === "topic" ? (
          ...
          <>
            <p className="text-xs text-[var(--muted-foreground)]">Upload a PDF — questions are generated from its content. Pick the class; difficulty is inferred from the document. The quiz subject comes from Step 1.</p>
        ...
        <p className="text-[11px] leading-relaxed text-[var(--muted-foreground)]">
          Generated via Skillship AI service · Gemini. Review every output before publishing.
        </p>
      </div>
    </motion.div>
  );
