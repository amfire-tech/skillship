    ...
    <div className="space-y-6">
        ...
        <div>
          ...
          {loading ? (
            ...
            <div className="mt-2 flex flex-wrap items-center gap-3">
              <h1 className="text-2xl font-semibold text-[var(--foreground)]">{item?.title ?? t("Untitled")}</h1>
              {item?.content_type && (
          ...
          {item?.subject && <p className="mt-1 text-sm text-[var(--muted-foreground)]">{item.subject} · {t("added")} {fmtDate(item.created_at)}</p>}
        </div>
        {url && (
      ...
      <div className="overflow-hidden rounded-2xl border border-[var(--border)] bg-white shadow-sm">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center gap-3 px-6 py-16 text-center">
            ...
            <p className="text-sm text-[var(--muted-foreground)]">{t("Preview unavailable for this file type.")}</p>
            <a href={url} target="_blank" rel="noreferrer" className="rounded-full bg-primary px-5 py-2 text-sm font-semibold text-white hover:opacity-90">{t("Download / Open")}</a>
          </div>
        ) : (
          <div className="flex flex-col items-center justify-center gap-2 px-6 py-16 text-center">
            <p className="text-sm text-[var(--muted-foreground)]">{t("No file attached to this resource.")}</p>
          </div>
        )}
      </div>
      ...
      {item?.description && (
        <div className="rounded-2xl border border-[var(--border)] bg-white p-6 shadow-sm">
          <h2 className="mb-2 text-sm font-semibold text-[var(--foreground)]">{t("About this content")}</h2>
