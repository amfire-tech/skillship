...
export default function PlannerPage() {
  ...
  async function toggleTask(task: ChecklistTask) {
    ...
    if (!res.ok) { toast("Couldn't update task", "error"); load(); }
  }

  return (
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="mx-auto max-w-3xl space-y-5">
      <div>
        <h1 className="text-xl font-bold tracking-tight text-[var(--foreground)]">My Daily Planner</h1>
        <p className="mt-0.5 text-sm text-[var(--muted-foreground)]">Your day-by-day plan toward your career objective. Tick off each day as you go.</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-dashed border-[var(--border)] py-12 text-center">
          <p className="text-sm text-[var(--muted-foreground)]">You don't have a daily plan yet.</p>
          <p className="mt-1 text-xs text-[var(--muted-foreground)]">Open a saved roadmap and create your 30-day plan.</p>
          <Link href="/dashboard/student/roadmaps" className="mt-3 inline-flex h-9 items-center rounded-full bg-gradient-to-r from-primary to-accent px-5 text-xs font-semibold text-white">Go to Saved Roadmaps</Link>
        </div>
      )}
    </motion.div>
  );
